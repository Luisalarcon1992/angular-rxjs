...
import { IStarWars } from '../ejercicios-resueltos/06.interface';
...
(() =>{
    ...
    const getRequest = ( url: string ) => ajax.getJSON<IStarWars>(url);
    // ==================================================================

    // Realizar el llamado al URL para obtener a Luke Skywalker
    const personaje = '/people/2/';

    getRequest(`${SW_API}${personaje}`).pipe(
        // Realizar los operadores respectivos aquí
        //switchMap( resp => getRequest(resp.species[0])),
        switchMap( resp => zip(getRequest(resp.species[0]), of(resp)) ),
        map(([especie, personaje])=> {
            return {
                especie,
                personaje
            }
